...
public class S3StorageConfigurationProperties implements StorageConfigurationProperties {
    ...
    public S3StorageConfigurationProperties(@NotNull final URI uri) {

        this.uri = uri;
        this.serviceEndpoint = UriComponentsBuilder.newInstance().scheme(uri.getScheme()).host(uri.getHost()).port(uri.getPort()).build().toUri();
        final Map<String, String> queryParamsMap = UriComponentsBuilder.fromUri(uri).build().getQueryParams().toSingleValueMap();
        // final Map<String, String> queryParamsMap = new URIBuilder(uri).getQueryParams().stream()
        //     .collect(Collectors.toMap(pair -> pair.getName().toLowerCase(Locale.ENGLISH), NameValuePair::getValue, (first, second) -> second));
        ...
        this.storageId = UriComponentsBuilder.fromUri(this.serviceEndpoint).path(this.region).path("/").path(this.bucket).toUriString();
    }
    ...
    //     final String queryValue = index > 0 && queryParameter.length() > index + 1 ? queryParameter.substring(index + 1) : null;
