...
abstract class StorageRepositoryTest {
    ...
    private final StorageRepository<? extends WebFile, ?> repository;
    ...
    @Test
    public void saveByFileType_validArgs_Ok() throws IOException {
        final Instant now = Instant.now().truncatedTo(ChronoUnit.MILLIS);
        final WebFile saved =
            this.repository.save(WebFileType.VALID_TEXT, "temp-path/test.txt", "text/plain", "Test".getBytes(StandardCharsets.UTF_8),
                Map.of("key", "value"));
        Assertions.assertEquals(this.repository.getConfiguration().getEndpoint() + "/test/files/temp_path/0/c/0cbc6611f5540bd0809a388dc95a615b/test.txt",
    ...
    @Test
    public void saveMultipart_validArgs_Ok() throws IOException {
        Assertions.assertThrows(IllegalArgumentException.class, () -> this.repository.save(WebFileType.VALID_TEXT, List.of()));

        final List<MockMultipartFile> multipartFiles =
            List.of(new MockMultipartFile("unused", "temp-path/test.txt", "text/plain", "Test".getBytes(StandardCharsets.UTF_8)),
                new MockMultipartFile("unused", "temp-path/test.txt", "text/plain", "Test".getBytes(StandardCharsets.UTF_8)));

        Assertions.assertEquals(1, this.repository.save(WebFileType.VALID_TEXT, List.of(multipartFiles.get(0))).size());
    ...
    @Test
    public void service_validArgs_validatorsExist() {
